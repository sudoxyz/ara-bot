...
module.exports = class rest4 extends WeebCommand {
  constructor(client) {
    super(client, {
      name: "human2",
      description: `Sends a user to #human-rights-2\n
                         Aliases: r4, h2, human2, rest4\n
                         Available only to Staff and Mods`,
      usage: `${client.config.prefix}rest4 @mention/id`,
      aliases: ["r4", "h2", "human2", "rest4"],
    ...
    this.client.dbusers.updateRestricted4(user.id, true);
    if(user.voice){
      await user.voice.disconnect().catch(console.error);
    }
    await user.roles.add(IDs.role.restricted4);

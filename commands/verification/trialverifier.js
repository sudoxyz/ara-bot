...
module.exports = class trialverifier extends WeebCommand {
  constructor(client) {
    super(client, {
      name: 'trialverifier',
      description: `Gives a user the trial verifier role`,
      usage: `${client.config.prefix}trialverifier @mention/id`,
      aliases: [],
      permLevel: 'Vegan',
    });
  }
  ...
  async run(message, args) {
    try {
      ...
      if (member.roles.cache.has('982635638010572850')) {
        return member.roles
          .remove('982635638010572850')
          .then(() => message.react('✅'));
      }
      const b12chat =
        message.guild.channels.cache.get('873215538627756072');
      const helpMessage = `Hi ${member}! You have been given the ${
        message.guild.roles.cache.get('982635638010572850').name
      } role by ${
        message.member
  ...
  giveModRole(member) {
    return member.roles.add('982635638010572850');
  }
  ...
  isInvalidTarget(member) {
    return !member || member?.roles.cache.has('982635638010572850');
  }

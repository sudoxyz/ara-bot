...
module.exports = class vega extends WeebCommand {
  constructor(client) {
    super(client, {
      name: 'vega',
      description: `Gives a user the veg access role`,
      usage: `${client.config.prefix}vega @mention/id`,
      aliases: ['vegaccess'],
      permLevel: 'Vegan',
    });
  }
  ...
  async run(message, args) {
    try {
      ...
      if (member.roles.cache.has('841767594663215135')) {
        return member.roles
          .remove('841767594663215135')
          .then(() => message.react('✅'));
      }
      ...
      const helpMessage = `Hi ${member}! You have been given the ${
        message.guild.roles.cache.get('841767594663215135').name
      } role by ${
        message.member
  ...
  giveVegSupportRole(member) {
    return member.roles.add('841767594663215135');
  }

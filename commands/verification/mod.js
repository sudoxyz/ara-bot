...
module.exports = class mod extends WeebCommand {
  constructor(client) {
    super(client, {
      ...
      usage: `${client.config.prefix}mod @mention/id`,
      aliases: ['moderator'],
      permLevel: 'Vegan',
    });
  }
  ...
  async run(message, args) {
    try {
      ...
      if (member.roles.cache.has('826157475815489598')) {
        return member.roles
          .remove('826157475815489598')
          .then(() => message.react('✅'));
      }
      const b12chat =
        message.guild.channels.cache.get('880864978851221524');
      const helpMessage = `Hi ${member}! You have been given the ${
        message.guild.roles.cache.get('826157475815489598').name
      } role by ${
        message.member
  ...
  giveModRole(member) {
    return member.roles.add('826157475815489598');
  }
  ...
  isInvalidTarget(member) {
    return !member || member?.roles.cache.has('826157475815489598');
  }
